...
var loadUser = createAction("[Auth] Load User");
var loadUserSuccess = createAction("[Auth] Load User Success", props());
var loadUserFailure = createAction("[Auth] Load User Failure", props());
var refreshToken = createAction("[Auth] Refresh Token");
var refreshTokenSuccess = createAction("[Auth] Refresh Token Success", props());
var refreshTokenFailure = createAction("[Auth] Refresh Token Failure");
...
var authReducer = createReducer(initialState, on(login, (state) => __spreadProps(__spreadValues({}, state), {
  loading: true,
  error: null
})), on(loginSuccess, (state, { response }) => __spreadProps(__spreadValues({}, state), {
  user: response.user,
  ...
  loading: true,
  error: null
})), on(registerSuccess, (state, { response }) => __spreadProps(__spreadValues({}, state), {
  user: response.user,
  token: response.token,
  isAuthenticated: true,
  loading: false,
  ...
  loading: false,
  error
})), on(logout, () => initialState), on(loadUserSuccess, (state, { user }) => __spreadProps(__spreadValues({}, state), {
  user,
  isAuthenticated: true
})), on(loadUserFailure, (state) => __spreadProps(__spreadValues({}, state), {
  user: null,
  isAuthenticated: false
})), on(refreshTokenSuccess, (state, { response }) => __spreadProps(__spreadValues({}, state), {
  user: response.user,
  token: response.token,
  isAuthenticated: true
})), on(refreshTokenFailure, () => initialState));
var authFeature = createFeature({
  name: "auth",
  reducer: authReducer
});
var { selectAuthState, selectUser, selectToken, selectIsAuthenticated, selectLoading, selectError } = authFeature;

// src/app/store/auth/auth.selectors.ts
var selectAuthState2 = authFeature.selectAuthState;
var selectUser2 = authFeature.selectUser;
var selectToken2 = authFeature.selectToken;
var selectIsAuthenticated2 = authFeature.selectIsAuthenticated;
var selectLoading2 = authFeature.selectLoading;
var selectError2 = authFeature.selectError;
var selectDisplayName = createSelector(authFeature.selectUser, (user) => user?.displayName || user?.email || "Usu\xE1rio");
var selectIsSuperAdmin = createSelector(authFeature.selectUser, (user) => user?.isSuperAdmin || false);
var selectCanManagePermissions = createSelector(authFeature.selectUser, (user) => user?.canManagePermissions || false);
var selectCanViewAdmin = createSelector(authFeature.selectUser, (user) => user?.canViewAdmin || false);
var selectCanViewReports = createSelector(authFeature.selectUser, (user) => user?.canViewReports || false);
var selectCanViewActivityHistory = createSelector(authFeature.selectUser, (user) => user?.canViewActivityHistory || false);
var selectCanEdit = createSelector(authFeature.selectUser, (user) => user?.isEditor || user?.isSuperAdmin || false);
var selectCanRevert = createSelector(authFeature.selectUser, (user) => user?.canRevert || user?.isSuperAdmin || false);
...
export {
  ...
  refreshTokenFailure,
  authFeature,
  selectUser2 as selectUser,
  selectIsAuthenticated2 as selectIsAuthenticated,
  selectLoading2 as selectLoading,
  selectError2 as selectError,
  selectDisplayName
};
...
@angular/core/fesm2022/rxjs-interop.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-64SJ7QU4.js.map
